...
function RegistrationChgWording() {
    try {
        DYO.smartObject("RegistrationChgWording", { target: "dy_RegistrationChgWording", inline: true });
    } catch (e) {
        window.setTimeout(function () { RegistrationChgWording(); }, 2000);
    }
}
RegistrationChgWording();
...
function Alert(msg, onlyAtParent) {
	if (onlyAtParent) if (window.parent.location != window.location) {
...
function FMNavigateTo(url) {
	var w = window;
	if (w.parent != null) w = w.parent;
	w.location.href = url
}
function FMChgPgIdx(uiFieldId, pgIdx) {
	var ui = $("#" + uiFieldId);
	ui.val(pgIdx);
	return true
}
...
function newPage(url) {
	...
	if (pathInSplit[lastElement] == "") url = "." + url;
	window.location = url
}
function getLoading() {
	...
	$(".loadingGif").find("img").css("padding-top", h)
}

function refreshBalance() {
	if (window.opener) $(window.opener.document).find("#uiRefreshBalance").click();
	else $("#uiRefreshBalance").click()
}
...
function AlertIframe(frameWidth, frameHeight, frameLink, openImmediately, frameScrolling, frameOverlayClose, frameEscKey, closeButton) {
	...
	$("#colorboxIndicator").colorbox({
		...
		escKey: frameEscKey,
		onLoad: function() {
			if (closeButton == false) $("#cboxClose").remove()
		}
	})
}
...
function getLoginLoading() {
	var w = $(window).width() / 2;
	var h = $(window).height() / 2;
	$(".loadingLoginGif").css("display", "block");
	$(".loadingLoginGif").find("img").css("padding-top", h)
}
...
function PopupIovation(strURL, intShowCloseButton, intPageWidth, intPageHeight, strParenRedirectURL) {
	...
	$.colorbox({
		...
		onLoad: function() {
			if (!bolShowCloseButton) $("#cboxClose").remove()
		},
		onComplete: function() {
			$("#cboxClose").css({
				top: "55px",
		...
		onClosed: function() {
			if (strParenRedirectURL != "") window.parent.location = strParenRedirectURL
		}
	})
}
...
function mobileRedirect(mobilesite) {
	YUI({
		...
		if (isMobileBrowser()) {
			...
			if (/^desktop$/.test(param.view)) Y.Cookie.set(ForceDesktopViewCookieName, "1", {
				path: "/"
			});
			else if (!Y.Cookie.exists(ForceDesktopViewCookieName)) location = mobilesite
		}
	})
}
...
function formQuickRegistrationReferrer(site, source, medium) {
	...
	if (referrerLink != "") CreateCookie("qr_referrer", referrerLink, 3)
}
...
function getPokerLoading() {
	$(".loadingLoginGif").css("display", "block").css("z-index", 1E6).find("img").css("padding-top", $(window).height() / 2)
}
...
function PokerAuthentication() {
	...
	$.post("/Services/PokerService.ashx", {
		...
		if (data.result == "success") {
			...
			if (data.code == "9001") $("#PokerErrorMessage").text(PokerMemberExist);
			else if (data.code == "9002") $("#PokerErrorMessage").text(PokerLength);
			else if (data.code == "9003") $("#PokerErrorMessage").text(PokerLetterNickName);
			else if (data.code == "9004") $("#PokerErrorMessage").text(PokerMemberUnavailable);
			else if (data.code == "9998") $("#PokerErrorMessage").text(PokerNotResponding);
			else if (data.code == "9999") $("#PokerErrorMessage").text(PokerError);
			else $("#PokerErrorMessage").text(data.description);
			$(".login_message").show().delay(5E3).fadeOut("slow")
		}
		closePokerLoading()
	}, "json");
	return false
...
function openGameWindowBase(gamePath, height, width) {
	if (typeof popupWin != "object") popupWin = window.open(gamePath, "0", "toolbar=0,menubar=0,location=0,status=0,height=" + height + ",width=" + width + ",scrollbars=no,resizable=yes");
	else if (!popupWin.closed) popupWin.location.href = gamePath;
	else popupWin = window.open(gamePath, "0", "toolbar=0,menubar=0,location=0,status=0,height=" + height + ",width=" + width + ",scrollbars=no,resizable=yes");
	popupWin.focus()
}

function EnterSlotGame(getId, memberCode, isGameLock) {
	var UrlSlotGame = "/CasinoGames/SlotLobby.aspx?GameId=";
	if (isGameLock == "0") if (Boolean(isLogin) && currency == "krw") {
		...
		if (IsPlayFunReal == "btnPlayReal") GamePath = UrlSlotGame + GameId + "&ProductCode=" + ProviderCategory;
		else GamePath = UrlSlotGame + GameId + "&ProductCode=" + ProviderCategory + "&IsDemo=1";
		if (IsPlayFunReal == "btnPlayReal") if (!DialogManager_isLogin(memberCode)) {
...
function EnterGame(getId, memberCode, isGameLock) {
	...
	if (isGameLock == "0") {
		...
		if (typeof getId == "string" && getId.toLowerCase().indexOf("exchange") !== -1) {
		...
		} else if (getId.id.toLowerCase() == "btn_s1" || getId.id.toLowerCase() == "btn_s5") GamePath = UrlSlotGame + playboyID + "&ProductCode=" + playboyGameProvider;
		else if (checkParent.toLowerCase() == "popup") {
		...
		} else if (checkParent.toLowerCase() == "gamelist") {
			GameId = $("#" + getId.id).parent().find(".hdnGameId").attr("value");
			...
			if (isDownload == true) GamePath = UrlPathMainLobby + "productcode=ea&platform=download";
			else GamePath = UrlPath + GameId + "&ProductCode=" + ProviderCategory
		} else if (checkParent.toLowerCase() == "mainlobby") {
			ProviderCategory = $("#" + getId.id).attr("id");
			if (ProviderCategory == "MGSQF") GamePath = UrlPath + $("#hdnGameId_MGSQF_LiveRoulette").attr("value") + "&ProductCode=" + ProviderCategory;
			else GamePath = UrlPath + "0&ProductCode=" + ProviderCategory
		} else if (checkParent.toLowerCase() == "menu-provider") {
			ProviderCategory = $("#" + getId.id).attr("id");
			if (ProviderCategory == "MGSQF") GamePath = UrlPath + $("#hdnGameId_MGSQF_LiveRoulette").attr("value") + "&ProductCode=" + ProviderCategory;
		...
		} else if (checkParent.toLowerCase() == "dvlinkbanner") switch ($("#" + getId.id).parent().parent().find(".hdnGameProvider").attr("value")) {
		case "EA":
			GamePath = UrlPathMainLobby + "productcode=ea&platform=download";
			break;
		case "BOY":
			GamePath = UrlPathMainLobby + "productcode=boy&platform=download";
			break;
		case "AG":
			GamePath = UrlPathMainLobby + "productcode=ag&platform=download";
			break
		} else if (checkParent.toLowerCase() == "gamedetail" || checkParent.toLowerCase() == "dvrecommendedgames") {
			var isDownload = $("#" + getId.id).parent().find(".lnkPlayDownload").length;
			GameId = $("#" + getId.id).parent().find("#hdnGameId").attr("value");
			ProviderCategory = $("#" + getId.id).parent().find("#hdnGameProvider").attr("value");
			if (ProviderCategory == "BOY" && memberCode.length > 15) {
		...
		} else if (checkParent.toLowerCase() == "recommended") {
			GameId = $("#" + getId.id).parent().find("#hdnGameId").attr("value");
			ProviderCategory = $("#" + getId.id).parent().find("#hdnGameProvider").attr("value");
			GamePath = UrlPath + GameId + "&ProductCode=" + ProviderCategory
		} else if (checkParent.toLowerCase() == "map2") {
			ProviderCategory = $("#" + getId.id).attr("id");
			GamePath = UrlPathMainLobby + "ProductCode=" + ProviderCategory
		} else if (checkParent.toLowerCase() == "play-slot") {
			GameId = $("#" + getId.id).parent().find("input").attr("value");
			ProviderCategory = $("#" + getId.id).parent().find(".hdnGameProvider").attr("value");
			GamePath = UrlSlotGame + GameId + "&ProductCode=" + ProviderCategory
		} else if (checkParent && checkParent.toLowerCase().indexOf("starcasinogame") >= 0) {
			GameId = $("#" + getId.id).parent().find("#hdnGameId").attr("value");
			IsPlayFunReal = $("#" + getId.id).attr("class");
			if (GameId == null || GameId == "undefined") GameId = $("#" + getId.id).parent().parent().find(".hiddenGameId").attr("value");
			if (IsPlayFunReal == "btnPTRealPlay") GamePath = UrlPath + GameId + "&ProductCode=PT";
			else GamePath = UrlPath + GameId + "&ProductCode=PT&IsDemo=1"
		} else if (getId.id.toLowerCase() == "mgs_mppb" || getId.id.toLowerCase() == "mgs_mppb_cn" || getId.id.toLowerCase() == "btn_s1" || getId.id.toLowerCase() == "btn_s5") GamePath = UrlSlotGame + playboyID + "&ProductCode=" + playboyGameProvider;
			...
			if ($("#" + getId.id).hasClass("btnPlayReal")) GamePath = UrlSlotGame + GameId + "&ProductCode=" + ProviderCategory;
			else {
		...
		if (IsPlayFunReal == "btnPlayReal" || IsPlayFunReal == "btnPTRealPlay" || IsPlayFunReal == "Default") if (!DialogManager_isLogin(memberCode)) {
			EnsureLogin(window.location.href);
			return false
		}
		if (!Is_TS_User(memberCode, ProviderCategory)) return false;
		else if (!Is_AB_User(memberCode, ProviderCategory)) return false;
		if (ProviderCategory === "BFR") window.location.href = GamePath.toString();
		else if (getId.id.toLowerCase() == "btn_s1" || getId.id.toLowerCase() == "btn_s5") openGameWindow(GamePath.toString());
		else if (checkParent.toLowerCase() == "dvlinkbanner" || isDownload == true) window.location.href = GamePath;
		else if (ProviderCategory === "SUN" || ProviderCategory === "BFR") openGameWindowFullScreen(GamePath.toString());
		else openGameWindow(GamePath.toString());
		return false
	} else {
		DialogManager_Show(" ", window.gameLockMessage);
		$(".ui-dialog-titlebar").css("height", "15px");
...
function EnterGameTable(provider, gameId, memberCode, isGameLock, isSlot) {
	if (!isSlot) var urlPath = "/CasinoGames/Lobby.aspx?GameId=";
	else var urlPath = "/CasinoGames/SlotLobby.aspx?GameId=";
	var provCode = "&ProductCode=";
	var gamePath;
	if (!DialogManager_isLogin(memberCode)) {
		if (!isSlot) EnsureLogin(window.location.href);
		else EnsureSlotLogin(window.location.href);
		return false
	}
	if (!isSlot) if (!Is_TS_User(memberCode, provider)) return false;
...
function incrementalJackpot(gameId) {
	setInterval(function() {
		...
		if (checkVal > 0 || checkVal != "NaN") {
			convertVal = convertVal + .01;
			if (typeof accounting !== "undefined") $("#" + gameId).text(accounting.formatNumber(convertVal, 2, ",", "."))
		}
	}, 1E3)
}
...
function incrementalJackpotByClass(gameClass) {
	setInterval(function() {
		...
		if (checkVal > 0 || checkVal != "NaN") {
			convertVal = convertVal + .01;
			if (typeof accounting !== "undefined") $("." + gameClass).text(accounting.formatNumber(convertVal, 2, ",", "."))
		}
	}, 1E3)
}
...
function LoadGameDetails(id) {
	try {
		...
		if (window.gameObject != null) {
			for (var i in window.gameObject) if (window.gameObject[i].GameCode == id) {
				gameJson = window.gameObject[i];
				break
			}
			if (gameJson != null && dvGames.length > 0) {
				var z = dvGames.find("img.orignal");
				z.attr({
					"src": gameJson.GameImage,
					"alt": gameJson.GameAltImage
				});
				...
				for (var j = 0; j < dvGames.length; j++) if ($(dvGames[j]).parents().eq(2).index() === 0) {
					var item = $(dvGames[j]);
					item.removeClass("dvGames").addClass("dvMain").append($("<div>", {
						"class": "hotbanner"
					})).find("img.orignal").css({
						"height": "323px",
						"width": "450px"
					}).attr("src", gameJson.GameImage.replace(".jpg", "_b.jpg"));
...
function LoadSlotGameDetails(gameCode) {
	try {
		...
		if (window.slotGame != null) {
			for (var i in window.slotGame) if (window.slotGame[i].GameCode == gameCode) {
			...
			if (gameJson != null) {
				...
				if (!DialogManager_isLogin(window.memberCode)) gamename.attr({
					"class": "btnPlayFun"
				});
				else gamename.attr({
					"class": "btnPlayReal"
				...
				dvGames.show()
			} else dvGames.parent().remove()
		} else window.setTimeout(function() {
			LoadSlotGameDetails(gameCode)
		}, 2E3)
	} catch (e) {
		window.setTimeout(function() {
			LoadSlotGameDetails(gameCode)
...
function alignSubMenuToRight(parentElement, ofElement) {
	...
	$(parentElement).children().each(function(i, li) {
...
function popUpWindowManager(defaultUrl, titlePopup, widthPopup) {
	var pathname = window.location.pathname;  //ocation.pathname -- 返回URL的域名后的部分
	var pathInSplit = pathname.split("/");
	var lastElement = pathInSplit.length - 1;
