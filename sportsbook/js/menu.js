...
function initialMenu() {
    ...
    var menuVerifyKey = fFrame.document.getElementById(frameId).contentWindow.MenuVerifyKey;
    ...
    SetMenuVerifyKey(menuVerifyKey);
    setInterval("AutoRefreshMenuData()", MaxMilliSec);
    Tmpl_Initialed = true;

    ifrmaeresizt();
    menuTitle.style.display = '';
    if (fFrame.IsLogin && fFrame.EnableBettrade) {
      betlistmini = new ONEbook.MemberSite.BetlistMin(fFrame.BetTradeHubService);
      betlistmini.initialize();
    }
}
...
function ShowOdds(Market, SportType, DispVer, BetType) {
    ...
    if ((SportType < 180 || SportType > 186) && SportType != 252 && SportType != 154 && SportType > 0 && (Market == 'T' || Market == 'E' || Market == 'L')) {
        if (sGame == fFrame.LastSelectedMenu) {
    ...
    try {
        ...
        if (Market == 'P') {
            ...
            if ((SportType < 180 || SportType > 186) && SportType != 252 && SportType > 0) {
                if ((m_sports[SportType][Market] <= 0) && (m_sports[SportType]['OT'] > 0)) {
    ...
    if (spObj != null) {
        if (!CheckCountAndSetOtherItem(market, sport)) {
            ...
            if (market == 'L') {
                ...
                if (sport == '15X' || sport == 201 || sport == 161 || sport == 154) {
                    if (sport == '15X') {
...
function SetMenuItem(market) {
    ...
    if ((fFrame.IsLogin) && (fFrame.SiteId != "4200300") && (fFrame.SiteId != "4200400") && (objMPBettingPage.style.display == 'none')) {
...
function SetLastSelectedSport(market, IsAutoRefresh) {
    ...
    if ((market == 'E' && fFrame.LastSelectedSport == 0) || (market == 'E' && fFrame.LastSelectedSport >= 151 && fFrame.LastSelectedSport <= 186)) {
        fFrame.LastSelectedSport = -1;
    }
...
function SetMenuData(SportsArray, imgsrc) {
    ...
    if (fFrame.deeplink) {
        ...
        if (_LastSport < 180 || _LastSport > 186) {
...
function SwitchSymbol() {
    if (fFrame.LastSelectedSport > 0) {
        ...
        else if (fFrame.LastSelectedSport >= 180 && fFrame.LastSelectedSport <= 186) {
...
function SetOpenOE(vOlympic, vEuroCup) {
    var OpenOlympic = vOlympic,
        OpenEuroCup = vEuroCup;
    SetMenuDisplayCup(OpenOlympic, OpenEuroCup);
}


function SetMenuDisplayCup(openOlympic, openEuroCup) {
